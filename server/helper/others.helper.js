...
otherHelper.paginationSendResponse = (
  ...
  loggerHelper.requestLogger.debug({
    ...
    size: pagesize,
    totalData: totalData,
    dataCount: data ? (Array.isArray(data) ? data.length : 1) : 0
  }, 'Sending paginated API response');
...
otherHelper.getQuerySendResponse = async (
  ...
  try {
    ...
    loggerHelper.logDatabase('find', model.modelName, true, duration, {
      page,
      size,
      resultCount: pulledData.data.length,
      totalData: pulledData.totalData,
    ...
    loggerHelper.logDatabase('find', model.modelName, false, duration, {
      error: err.message,
      searchQuery: JSON.stringify(searchQuery)
    });
